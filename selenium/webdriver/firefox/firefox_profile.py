...
#("Addons must be added after starting the session")
...
class FirefoxProfile:
    ...
    @property
    #("The port is stored in the Service class")
    def port(self):
    ...
    def port(self, port) -> None:
        ...
        if not isinstance(port, int):
            raise Exception("Port needs to be an integer")
        try:
            port = int(port)
            if port < 1 or port > 65535:
                raise Exception("Port number must be in the range 1..65535")
    ...
    @property
    #("Allowing untrusted certs is toggled in the Options class")
    ...
    def accept_untrusted_certs(self, value) -> None:
        if not isinstance(value, bool):
            raise Exception("Please pass in a Boolean to this call")
